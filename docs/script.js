class Todo {
    selectors = {
        root: '[data-js-todo]',
        newTaskFrom: '[data-js-todo-new-task-form]',
    ...
    constructor() {
        ...
        this.newTaskFormElement = this.rootElement.querySelector(this.selectors.newTaskFrom)
    ...
    filter() {
        ...
        this.state.filtered.Items = this.state.items.filter(({ title }) => {
            const titleFromatted = title.toLowerCase()

            return titleFromatted.includes(queryFormatted)
        })
        this.render()
    }
